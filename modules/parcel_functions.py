...
def load_moist_adiabat_lookups(**kwargs):
    ...
    """
    this.moist_adiabat_lookup, this.moist_adiabats = moist_adiabat_tables(regenerate=False, cache=True, **kwargs)
...
def get_layer(dat, depth=100, vert_dim='model_level_number', interpolate=True):
    ...
    if interpolate:
        ...
        interp_level = log_interp(x=dat, at=top_pressure, coords=dat.pressure, dim=vert_dim)
        interp_level['pressure'] = top_pressure

        dat = insert_level(d=dat, level=interp_level, coords='pressure', vert_dim=vert_dim)
    else:
        top_pressure = bound_pressure(pressure=dat.pressure, bound=bottom_pressure - depth, vert_dim=vert_dim)
...
def most_unstable_parcel(dat, depth=300, vert_dim='model_level_number'):
    """Return the most unstable parcel.

    Parcel is that with an atmospheric layer from
    ...
    return layer.where(layer.pressure == pres).max(dim=vert_dim, keep_attrs=True)
...
def trapz(dat, x, dim, mask=None, only_positive=False, only_negative=False):
    """Perform trapezoidal rule integration along an axis.

    This is ala numpy.trapz.
    Estimates int y dx.
    ...
    Arguments:
        ...
        only_positive: Include only positive values of the area?
        only_negative: Include only negative values of the area?

    Returns: Integrated value along the axis.
...
def bound_pressure(pressure, bound, vert_dim='model_level_number'):
    """Calculate the bounding pressure in a layer.
    ...
    Arguments:
        ...
        vert_dim: Model vertical coordinate.
    ...
    return bounds.max(dim=vert_dim).squeeze(drop=True)


def mixed_parcel(pressure, temperature, dewpoint, depth=100, vert_dim='model_level_number'):
    """Fully mix a layer of given depth above the surface.

    Find the temparature, pressure and dewpoint of the parcel.
...
def dry_lapse(pressure, parcel_temperature, parcel_pressure=None, vert_dim='model_level_number'):
    """Calculate the temperature of a parcel raised dry-adiabatically (conserving potential temperature).
...
def moist_adiabat_tables(
    ...
    **kwargs,
    ...
    Returns: two DataArrays: 1) a lookup table of pressure/temperature vs. adiabat number, and
    2) a lookup table of adiabat number to temperature by pressure profiles.
...
def wet_bulb_temperature_fast(temperature, dewpoint):
    """Use a fast method to estimate wet bulb temperature.

    The method is the "1/3" rule discussed in Knox et al., 2017
    (https://doi.org/10.1175/BAMS-D-16-0246.1). Its error is shown in their Figure 3.

    Arguments:
        temperature: Temperature at each pressure level [K].
        dewpoint: Dewpoint temperatures [K].

    Returns: Estimated wet-bulb temperature.
...
def wet_bulb_temperature(pressure, temperature, dewpoint, vert_dim='model_level_number'):
    """Calculate wet-bulb pressure using "Normand's rule".

    See Knox et al., 2017 (https://doi.org/10.1175/BAMS-D-16-0246.1) for a description of the method.
    ...
    Returns: The wet bulb temperature of each point.

    """
    if pressure.chunks is not None:
        print(
            'WARNING: wet_bulb_temperatuer function uses a for loop that ',
    ...
    if vert_dim in pressure.coords:
        ...
        for v in pressure[vert_dim]:
            lcls = lcl(
                parcel_pressure=pressure.sel({vert_dim: v}),
            ...
            ml.loc[{vert_dim: v}] = (
                moist_lapse(pressure=p, parcel_temperature=lcls.lcl_temperature, parcel_pressure=lcls.lcl_pressure, vert_dim=vert_dim)
                .compute()
                .squeeze()
        ...
        ml = moist_lapse(pressure=pressure, parcel_temperature=lcls.lcl_temperature, parcel_pressure=lcls.lcl_pressure, vert_dim=vert_dim)
...
def moist_adiabat_lookup(
    pressure_levels=np.round(np.arange(1100, 2, step=-0.5), 1),
    ...
    Arguments:
        ...
        pres_step: (Positive) step size for pressure_levels.
        temp_step: (Positive) step for temperatures.

    Returns: two DataArrays: 1) a lookup table of pressure/temperature
    ...
    adiabat_lookup = adiabat_lookup.expand_dims({'pressure': pressure_levels, 'temperature': temperatures}).copy(deep=True)

    # Find the adiabat for each starting temperature.
    i = 1
    for parcel_temperature in temperatures:
        for offset in [0, temp_step / 2]:
            profile_temps = metpy.calc.moist_lapse(pressure=pressure_levels * units.hPa, temperature=(parcel_temperature + offset) * units.K).m
            ...
            pres_per_temp = np.interp(x=temperatures, xp=profile_temps[::-1], fp=pressure_levels[::-1], right=np.nan, left=np.nan)
            ...
            curves.append(xarray.Dataset({'temperature': (['pressure'], profile_temps)}, coords={'pressure': pressure_levels}))
            curves[-1]['adiabat'] = i
            i = i + 1
...
def moist_lapse(pressure, parcel_temperature, parcel_pressure=None, vert_dim='model_level_number', persist=True):
    """Return the temperature of parcels raised moist-adiabatically (assuming liquid saturation processes).

    Note: What is returned are approximate pseudo-adiabatic moist lapse rates, found using a
    lookup table.
    ...
    Returns: Temperature of each parcel lifted to each pressure level.
    ...
    adiabat_idx = this.moist_adiabat_lookup.sel({'pressure': parcel_pressure, 'temperature': parcel_temperature}, method='nearest')
    ...
    return out.where(~np.isnan(pressure))
...
def lcl(parcel_pressure, parcel_temperature, parcel_dewpoint):
    ...
    # before returning.
    valid_points = np.logical_not(np.logical_or(np.logical_or(np.isnan(parcel_pressure), np.isnan(parcel_temperature)), np.isnan(parcel_dewpoint)))
    ...
    def lcl_block(obj):
        press_lcl, temp_lcl = metpy.calc.lcl(pressure=obj.parcel_pressure, temperature=obj.parcel_temperature, dewpoint=obj.parcel_dewpoint)
        ...
        lcl_mixing_ratio = mixing_ratio(temperature=temp_lcl, dewpoint=temp_lcl, pressure=press_lcl)
        lcl_virt_temp = virtual_temperature(temperature=temp_lcl, mixing_ratio=lcl_mixing_ratio)

        return xarray.Dataset(
            {'lcl_pressure': (dims, press_lcl.m), 'lcl_temperature': (dims, temp_lcl.m), 'lcl_virtual_temperature': (dims, lcl_virt_temp.m)},
    ...
    out.lcl_virtual_temperature.attrs['long_name'] = 'Lifting condensation ' + 'level virtual temperature'
    out.lcl_virtual_temperature.attrs['units'] = 'K'

    # Only return valid points.
    return out.where(valid_points)
...
def mixing_ratio(temperature, dewpoint, pressure):
    ...
    Returns: Mixing ratio [kg kg-1].

    """
    relative_humidity = metpy.calc.relative_humidity_from_dewpoint(temperature=temperature, dewpoint=dewpoint)
    res = metpy.calc.mixing_ratio_from_relative_humidity(pressure=pressure, temperature=temperature, relative_humidity=relative_humidity)
...
def parcel_profile(pressure, parcel_pressure, parcel_temperature, parcel_dewpoint, vert_dim='model_level_number'):
    ...
    Returns: Dataset with the temperature of the parcel lifted from parcel_pressure to levels in pressures,
    plus the LCL pressure and temperature.
    ...
    # point to the LCL.
    below_lcl = dry_lapse(pressure=pressure, parcel_temperature=parcel_temperature, parcel_pressure=parcel_pressure, vert_dim=vert_dim)

    # Along the dry adiabat the parcel's mixing ratio remains constant.
    parcel_mixing_ratio = mixing_ratio(
    ...
    # temp/pressure.
    above_lcl = moist_lapse(pressure=pressure, parcel_temperature=out.lcl_temperature, parcel_pressure=out.lcl_pressure, vert_dim=vert_dim)

    # Above the LCL, the mixing ratio is the saturation mixing ratio.
    mixing_ratios = metpy.calc.saturation_mixing_ratio(total_press=pressure, temperature=above_lcl)
    ...
    mixing_ratios = mixing_ratios.where(pressure <= out.lcl_pressure, other=parcel_mixing_ratio)
    out['virtual_temperature'] = virtual_temperature(temperature=out.temperature, mixing_ratio=mixing_ratios)
...
def virtual_temperature(temperature, mixing_ratio, epsilon=0.608):
    ...
    Returns: The virtual temperature(s).
...
def parcel_profile_with_lcl(
    ...
    Returns: Dataset with the temperature of the parcel lifted from
    ...
    mix_ratio = mixing_ratio(temperature=temperature, dewpoint=dewpoint, pressure=pressure)
    virtual_temp = virtual_temperature(temperature=temperature, mixing_ratio=mix_ratio)

    environment = xarray.Dataset(
        {'temperature': temperature, 'virtual_temperature': virtual_temp, 'dewpoint': dewpoint, 'pressure': profile.pressure},
    )
    ...
    return add_lcl_to_profile(profile=profile, vert_dim=vert_dim, environment=environment, interpolator=lcl_interp)


def add_lcl_to_profile(profile, vert_dim='model_level_number', environment=None, interpolator='log'):
    ...
    Returns: A new profile object with LCL pressure and temperatures
    ...
    level = xarray.Dataset(
        {'pressure': profile.lcl_pressure, 'temperature': profile.lcl_temperature, 'virtual_temperature': profile.lcl_virtual_temperature},
    )
    ...
    if environment is not None:
        ...
        if interpolator == 'linear':
            interp_level = linear_interp(x=environment, coords=environment.pressure, at=level.pressure, dim=vert_dim)
        elif interpolator == 'log':
            interp_level = log_interp(x=environment, coords=environment.pressure, at=level.pressure, dim=vert_dim)
        ...
        if 'virtual_temperature' in interp_level:
            ...
            mix_ratio = mixing_ratio(temperature=interp_level.temperature, dewpoint=interp_level.dewpoint, pressure=interp_level.pressure)
            interp_level['virtual_temperature'] = virtual_temperature(temperature=interp_level.temperature, mixing_ratio=mix_ratio)
...
def insert_level(d, level, coords, vert_dim='model_level_number', fill_value=-999):
    ...
    Arguments:
        ...
        fill_value: Value to use as a filler.
    ...
    """
    assert np.all(np.abs(d[vert_dim].diff(dim=vert_dim)) == 1), 'Vert_dim index increments ' + 'must all be 1.'
    ...
    return out.where(out != fill_value, other=np.nan)
...
def find_intersections(x, a, b, dim, log_x=False):
    ...
    Returns: Dataset containing x, y coordinates for all intersections,
    ...
    before_intersects = xarray.concat([xarray.zeros_like(a.isel({dim: 0})), after_intersects], dim=dim)
    ...
    return out.rename({dim: 'offset_dim'})


def lfc_el(pressure, parcel_temperature, temperature, lcl_pressure, lcl_temperature, vert_dim='model_level_number'):
    ...
    Returns: DataArray with LFC pressure (lfc_pressure) and temperature
             (lfc_temperature).

    """
    # Find all intersections between parcel and environmental
    # temperatures by pressure.
    intersections = find_intersections(x=pressure, a=parcel_temperature, b=temperature, dim=vert_dim, log_x=True)
    ...
    # LFC above it.
    intersections = intersections.where((temperature.isel({vert_dim: 0}) != parcel_temperature.isel({vert_dim: 0})), other=intersections_above)
    ...
    out['lfc_temperature'] = intersections.increasing_y.where(intersections.increasing_x == out.lfc_pressure).max(dim='offset_dim')
    ...
    temps_available = np.logical_and(~np.isnan(parcel_temperature), ~np.isnan(temperature))
    ...
    pos_parcel = parcel_temperature.where(above_lcl) > temperature.where(above_lcl)
    ...
    out['lfc_pressure'] = lcl_pressure.where(replace_with_lcl, other=out.lfc_pressure)
    out['lfc_temperature'] = lcl_temperature.where(replace_with_lcl, other=out.lfc_temperature)
...
def trap_around_zeros(x, y, dim, log_x=True, start=0):
    ...
    Returns: a Dataset containing the areas and x coordinates for each
    ...
    def calc_areas(x, y, mask, shift_x=0):
        areas = xarray.Dataset({'area': xarray.zeros_like(mask), 'dx': xarray.zeros_like(mask), 'x': xarray.zeros_like(mask)})
        ...
        return areas.reset_coords(drop=True)
...
def cape_cin_base(
    ...
    Returns: Dataset with convective available potential energy (cape)
          and convective inhibition (cin), both in J kg-1.

    """
    # Where the EL is nan, use the highest (lowest-pressure) value as
    # the EL.
    el_pressure = pressure.min(dim=vert_dim).where(np.isnan(el_pressure), other=el_pressure)

    # Difference between the parcel path and measured temperature
    # profiles.
    temp_diffs = xarray.Dataset({'temp_diff': (parcel_temperature - temperature), 'pressure': pressure, 'log_pressure': np.log(pressure)})
    ...
    areas_around_zeros, trapz_mask = trap_around_zeros(x=temp_diffs.pressure, y=temp_diffs.temp_diff, dim=vert_dim, log_x=True)
    ...
    cape = mpconsts.Rd.m * trapz(dat=diffs_lfc_to_el, x='log_pressure', dim=vert_dim, mask=trapz_mask, only_positive=pos_cape_neg_cin)
    ...
    cin = mpconsts.Rd.m * trapz(dat=temp_diffs_surf_to_lfc, x='log_pressure', dim=vert_dim, mask=trapz_mask, only_negative=pos_cape_neg_cin)
...
def cape_cin(
    ...
    **kwargs,
    ...
    Wraps finding of LFC and parcel profile
    ...
    Arguments:
        ...
        virtual_temperature_correction: Use virtual temperature correction?
    ...
    profile = parcel_profile_with_lcl(
        pressure=pressure,
    ...
    if not virtual_temperature_correction:
        ...
        cape_cin.attrs['correction'] = 'Virtual temperature correction not used ' + 'in CAPE/CIN calculations.'
    else:
        # Calculate LFC and EL.
        parcel_lfc_el = lfc_el(
            pressure=profile.pressure,
...
def from_most_unstable_parcel(pressure, temperature, dewpoint, vert_dim='model_level_number', depth=300):
    """Select pressure and temperature data at and above the most unstable parcel within the first x hPa above the surface.
    ...
    Returns: subset pressure, subset temperature, subset dewpoint,
    ...
    unstable_layer = most_unstable_parcel(dat=dat, depth=depth, vert_dim=vert_dim)
...
def most_unstable_cape_cin(pressure, temperature, dewpoint, vert_dim='model_level_number', depth=300, prefix=None, **kwargs):
    """Calculate CAPE and CIN for the most unstable parcel within a given depth above the surface..
...
def mix_layer(pressure, temperature, dewpoint, vert_dim='model_level_number', depth=100, load=True):
    ...
    Returns: pressure, temperature, and dewpoint profiles and the mixed layer parcel.

    """
    # Mix the lowest x hPa.
    mp = mixed_parcel(pressure=pressure, temperature=temperature, dewpoint=dewpoint, depth=depth, vert_dim=vert_dim)
...
def mixed_layer_cape_cin(pressure, temperature, dewpoint, vert_dim='model_level_number', depth=100, prefix=None, **kwargs):
    ...
    """
    pressure, temperature, dewpoint, mp = mix_layer(pressure=pressure, temperature=temperature, dewpoint=dewpoint, vert_dim=vert_dim, depth=depth)
...
def lifted_index(profile, vert_dim='model_level_number', description=None, prefix=None):
...
def deep_convective_index(pressure, temperature, dewpoint, lifted_index, vert_dim='model_level_number', description=None, prefix=None):
...
def min_conv_properties(dat, vert_dim='model_level_number'):
    ...
    temp_500 = isobar_temperature(pressure=dat.pressure, temperature=dat.temperature, isobar=500, vert_dim=vert_dim)
...
def conv_properties(dat, vert_dim='model_level_number', ignore_nans=False):
    ...
    valid_points = np.logical_and(~np.isnan(dat.dewpoint).any(vert_dim), ~np.isnan(dat.pressure).any(vert_dim))
    valid_points = np.logical_and(valid_points, ~np.isnan(dat.temperature).any(vert_dim))
    valid_points = np.logical_and(valid_points, ~np.isnan(dat.specific_humidity).any(vert_dim))
    ...
    mu_li = lifted_index(
        ...
        vert_dim=vert_dim,
        prefix='mu',
        description=('Lifted index using most-unstable ' + 'parcel in lowest 250 hPa.'),
    )
    mixed_li_100 = lifted_index(
        profile=mixed_profile_100,
        vert_dim=vert_dim,
    ...
    mixed_li_50 = lifted_index(
        profile=mixed_profile_50,
        vert_dim=vert_dim,
        prefix='mixed_50',
        description=('Lifted index using fully-mixed ' + 'lowest 50 hPa parcel.'),
    )

    print('Calculating deep convective indices...')
    mu_dci = deep_convective_index(
        pressure=dat.pressure,
        temperature=dat.temperature,
        dewpoint=dat.dewpoint,
        ...
        vert_dim=vert_dim,
        prefix='mu',
        description=('Deep convective index using most-unstable ' + 'parcel in lowest 250 hPa.'),
    )
    mixed_dci_100 = deep_convective_index(
        pressure=dat.pressure,
        temperature=dat.temperature,
        dewpoint=dat.dewpoint,
    ...
    mixed_dci_50 = deep_convective_index(
        pressure=dat.pressure,
        temperature=dat.temperature,
        dewpoint=dat.dewpoint,
        ...
        vert_dim=vert_dim,
        prefix='mixed_50',
        description=('Deep convective index using fully-mixed ' + 'lowest 50 hPa parcel.'),
    )

    print('Calculating mixing ratio of most unstable parcel...')
    mu_mixing_ratio = metpy.calc.mixing_ratio_from_specific_humidity(
        specific_humidity=metpy.calc.specific_humidity_from_dewpoint(pressure=mu_parcel.pressure * units.hPa, dewpoint=mu_parcel.dewpoint * units.K),
    ).metpy.dequantify()
...
def lapse_rate(pressure, temperature, height, from_pressure=700, to_pressure=500, vert_dim='model_level_number'):
    ...
    Returns: Lapse rate between two levels at each point [K km-1].

    """
    from_temperature = log_interp(x=temperature, coords=pressure, at=from_pressure, dim=vert_dim)
    to_temperature = log_interp(x=temperature, coords=pressure, at=to_pressure, dim=vert_dim)
    from_height = log_interp(x=height, coords=pressure, at=from_pressure, dim=vert_dim) / 1000
    to_height = log_interp(x=height, coords=pressure, at=to_pressure, dim=vert_dim) / 1000
...
def freezing_level_height(temperature, height, vert_dim='model_level_number', agl=False):
    ...
    Returns: Freezing level height [m].
    ...
    flh.attrs['long_name'] = 'Freezing-level height'
    flh.attrs['description'] = 'Height of zero degree dry-bulb temperature isotherm.'
    if agl:
        flh.attrs['description'] = 'Height of zero degree dry-bulb temperature isotherm above ground level.'
    flh.attrs['units'] = 'm'
...
def melting_level_height(pressure, temperature, dewpoint, height, fast=True, vert_dim='model_level_number'):
    ...
    Arguments:
        ...
        dewpoint: Dewpoint temperatures [K].
        height: Height of each level [m].
        fast: Use fast wet-bulb temperature calculation?
        vert_dim: The vertical dimension to operate on.

    Returns: Melting-level height [m], and the wet bulb temperature of each point.
    ...
    if fast:
        ...
        wb = wet_bulb_temperature(pressure=pressure, temperature=temperature, dewpoint=dewpoint, vert_dim=vert_dim)

    mlh = freezing_level_height(temperature=wb, height=height, vert_dim=vert_dim)

    mlh.attrs['long_name'] = 'Melting-level height'
    mlh.attrs['description'] = 'Height of zero degree wet-bulb temperature isotherm.'
...
def wind_shear(surface_wind_u, surface_wind_v, wind_u, wind_v, height, shear_height=6000, vert_dim='model_level_number'):
    """Calculate wind shear.

    Arguments:
        surface_wind_u: U component of surface wind.
        surface_wind_v: V component of surface wind.
        wind_u: U component of above-surface wind.
        wind_v: V component of above-surface wind.
        height: The height of every coordinate in wind_u and wind_v.
        shear_height: The wind height to subtract from surface wind [m].
        vert_dim: Name of vertical dimension.

    Returns: Wind shear = wind at shear_height - wind at surface.
...
def significant_hail_parameter(mucape, mixing_ratio, lapse, temp_500, shear, flh_agl):
    ...
    As given at https://www.spc.noaa.gov/exper/mesoanalysis/help/help_sigh.html
    ...
    Returns: Significant hail parameter.
    ...
    shear = shear.where(shear >= 7).where(shear <= 27)  # noqa: PLR2004
    mixing_ratio = mixing_ratio.where(mixing_ratio >= 11).where(mixing_ratio <= 13.6)  # noqa: PLR2004
    temp_500 = temp_500.where(temp_500 <= -5.5, other=-5.5)  # noqa: PLR2004
    ...
    ship = ship.where(mucape >= 1300, other=ship * (mucape / 1300))  # noqa: PLR2004
    ship = ship.where(lapse >= 5.8, other=ship * (lapse / 5.8))  # noqa: PLR2004
    ship = ship.where(flh_agl >= 2400, other=ship * (flh_agl / 2400))  # noqa: PLR2004
...
def storm_proxies(dat):
    ...
    # Proxy calculations.
    # Craven 2004.
    out['proxy_Craven2004'] = (dat.mixed_100_cape * dat.S06) >= 20000  # noqa: PLR2004

    # Kunz 2007.
    out['proxy_Kunz2007'] = np.logical_or(dat.mixed_100_lifted_index <= -2.07, np.logical_or(dat.mu_cape >= 1474, dat.mixed_100_dci >= 25.7))  # noqa: PLR2004

    # Trapp 2007.
    out['proxy_Trapp2007'] = np.logical_and(dat.mixed_100_cape * dat.S06 >= 10000, dat.mixed_100_cape >= 100)  # noqa: PLR2004
    out['proxy_Trapp2007'] = np.logical_and(out.proxy_Trapp2007, dat.S06 >= 5)  # noqa: PLR2004
    out['proxy_Trapp2007'] = np.logical_and(out.proxy_Trapp2007, dat.positive_shear)

    # Marsh 2009.
    out['proxy_Marsh2009'] = (dat.mixed_100_cape * dat.S06) >= 10000  # noqa: PLR2004

    # Allen 2011.
    out['proxy_Allen2011'] = dat.mixed_50_cape * dat.S06**1.67 >= 25000  # noqa: PLR2004

    # Allen 2014.
    out['proxy_Allen2014'] = np.logical_and(out.proxy_Allen2011, dat.mixed_50_cin > -25)  # noqa: PLR2004
    out['proxy_Allen2014'] = np.logical_and(out.proxy_Allen2014, dat.S06 > 7.5)  # noqa: PLR2004
    out['proxy_Allen2014'] = np.logical_and(out.proxy_Allen2014, dat.lapse_rate_700_500 < -6.5)  # noqa: PLR2004

    # Eccel 2012.
    out['proxy_Eccel2012'] = np.logical_and(dat.mixed_100_cape * dat.S06 > 10000, dat.mixed_100_cin > -50)  # noqa: PLR2004

    # Mohr 2013.
    out['proxy_Mohr2013'] = np.logical_or(dat.mixed_100_lifted_index <= -1.6, dat.mixed_100_cape >= 439)  # noqa: PLR2004
    out['proxy_Mohr2013'] = np.logical_or(out.proxy_Mohr2013, dat.mixed_100_dci >= 26.4)  # noqa: PLR2004

    # Significant hail parameter.
    out['ship'] = significant_hail_parameter(
        mucape=dat.mu_cape,
