...
def moist_adiabat_tables(regenerate=False, cache=True, chunks=None,
    ...
    if not regenerate:
        adiabat_lookup = xarray.open_dataset(lookup_cache, chunks=chunks).load()
        adiabats = xarray.open_dataset(adiabats_cache, chunks=chunks).load()
...
def moist_lapse(pressure, parcel_temperature, moist_adiabat_lookup,
...
def parcel_profile(pressure, parcel_pressure, parcel_temperature,
...
def mixed_layer_cape_cin(pressure, temperature, dewpoint, moist_adiabat_lookup,
                         moist_adiabats, vert_dim='model_level_number',
...
def conv_properties(dat, moist_adiabat_lookup, moist_adiabats,
                    vert_dim='model_level_number'):
    """
