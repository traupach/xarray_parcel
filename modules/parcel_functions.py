...
def most_unstable_parcel(dat, depth=300, drop=False, vert_dim='model_level_number'):
    ...
    requested bottom and depth. No interpolation is performed. If there are multiple
    'most unstable' parcels, return the first in the vertical array.
    ...
    assert np.all(layer.where(layer.pressure == pres).count(vert_dim) == 1), ('Vertical ' + 
                                                                              'pressures ' +
                                                                              'not unique.')
    most_unstable = layer.where(layer.pressure == pres).max(dim=vert_dim, keep_attrs=True)
...
def moist_adiabat_tables(regenerate=False, cache=True, chunks=None,
    ...
    if not regenerate:
        adiabat_lookup = xarray.open_dataset(lookup_cache, chunks=chunks)
        adiabats = xarray.open_dataset(adiabats_cache, chunks=chunks)
...
def moist_lapse(pressure, parcel_temperature, moist_adiabat_lookup, moist_adiabats,
                parcel_pressure=None, vert_dim='model_level_number', chunks=50):
    """
    ...
    Arguments:
        ...
        - parcel_pressure: Parcel pressure before lifting. Defaults to lowest vertical level.
        - vert_dim: The name of the vertical dimension.
        - chunks: Chunk size to use in non-vertical dimensions if Dask is being used.
...
def conv_properties(dat, moist_adiabat_lookup, moist_adiabats, vert_dim='model_level_number'):
    """
    ...
    Returns: 
        ...
        - Dataset containing mixed parcel (100 hPa depth) cape and cin, most 
          unstable parcel (250 hPa depth) cape and cin, lifted index and deep
          convective index for each point.
    """
