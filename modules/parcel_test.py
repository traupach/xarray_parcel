"""Routines to test xarray-enabled versions of MetPy functions for atmospheric parcel calculations."""
...
def compare(x, y, name, tolerance=1e-5):
    ...
    non_zero_y = y.where(y != 0)

    diffs = np.abs(x - y)
    reldiffs = np.abs(diffs / non_zero_y * 100)
    max_rel_diff = np.round(np.max(reldiffs).values, 2)
    max_diff = np.round(np.max(diffs).values, 5)

    comp = max_diff < tolerance
    if not comp:
        name_and_unit = x.attrs['long_name'] + ' [' + x.attrs['units'] + '] (' + name + ')' if 'long_name' in x.attrs else name + ' [?]'
...
def lcl_serial(parcel_pressure, parcel_temperature, parcel_dewpoint, x_dim='longitude', y_dim='latitude'):
    """Wrap the metpy lcl function, for testing.
    ...
    Arguments:
        ...
        x_dim: Name of x coordinate dimension.
        y_dim: Name of y coordinate dimension.
    ...
    for x in parcel_pressure[x_dim].values:
        for y in parcel_pressure[y_dim].values:
            ...
            lcl = xarray.Dataset({'lcl_pressure': press_lcl.m, 'lcl_temperature': temp_lcl.m, 'lcl_virtual_temperature': np.nan})
...
def moist_lapse_serial(
    ...
    """Vectorise and wrap the metpy serial implementation of moist_lapse, for testing.
        ...
        for y in pressure[y_dim].values:
            ...
            if temp.size > 1:
                ...
                for p in np.arange(temp.size):
                    moist[p] = moist_lapse_single_point(pressure=pres[p], parcel_temperature=temp[p], parcel_pressure=ref_pressure[p])
            else:
                moist = moist_lapse_single_point(pressure=pres, parcel_temperature=temp, parcel_pressure=ref_pressure).data

            if moist.shape == ():
                to_add = xarray.Dataset(
                    data_vars={'temperature': (vert_dim, [moist])},
                    coords={vert_dim: [pressure[vert_dim].values[0]], x_dim: x, y_dim: y},
                )
            else:
                to_add = xarray.Dataset(data_vars={'temperature': ([vert_dim], moist)}, coords={vert_dim: pressure[vert_dim], x_dim: x, y_dim: y})
...
def moist_lapse_single_point(pressure, parcel_temperature, parcel_pressure=None, vert_dim='model_level_number'):
    """Wrap the metpy implementation of moist_lapse, for testing purposes, for a single point.
    ...
    moist = metpy.calc.moist_lapse(pressure=pres, temperature=temp, reference_pressure=ref_pressure).m
    ...
    if pressure.size > 1:
        ...
        out = xarray.Dataset({'temperature': (vert_dim, moist)}, coords={vert_dim: pressure[vert_dim]})
    else:
        out = xarray.Dataset({'temperature': (vert_dim, [moist])}, coords={vert_dim: [pressure[vert_dim].values[0]]})
...
def surface_cape_serial(dat):
    ...
    for lat in dat.latitude.values:
        for lon in dat.longitude.values:
            ...
            spec_hum = dat.sel(latitude=lat, longitude=lon).specific_humidity.values * units(dat.specific_humidity.attrs['units'])

            # Dewpoint.
            dewpoint = metpy.calc.dewpoint_from_specific_humidity(pressure=pres, temperature=temp, specific_humidity=spec_hum).to('K')

            # Surface-based CAPE and CIN.
            surface_cape, surface_cin = metpy.calc.surface_based_cape_cin(pressure=pres, temperature=temp, dewpoint=dewpoint)
            out.append(xarray.Dataset(data_vars={'cape': surface_cape.m, 'cin': surface_cin.m}, coords={'latitude': lat, 'longitude': lon}))
...
def surface_cape_vector(dat):
    ...
    """
    # Dewpoints.
    dewpoint = metpy.calc.dewpoint_from_specific_humidity(pressure=dat.pressure, temperature=dat.temperature, specific_humidity=dat.specific_humidity)
    dewpoint = dewpoint.metpy.convert_units('K')
    dewpoint = dewpoint.metpy.dequantify()

    # CAPE and CIN.
    out, _ = parcel.surface_based_cape_cin(pressure=dat.pressure, temperature=dat.temperature, dewpoint=dewpoint)
    return out.load()
...
def conv_properties_metpy_serial(dat):
    ...
    for lat in dat.latitude.values:
        for lon in dat.longitude.values:
            ...
            mp_pres, mp_temp, mp_dewpoint = metpy.calc.mixed_parcel(pressure=pres, temperature=temp, dewpoint=dewpoint, depth=100 * units.hPa)
            ...
            p, t, td, mixed_profile = metpy.calc.parcel_profile_with_lcl(pressure=pres_prof, temperature=temp_prof, dewpoint=dew_prof)

            # Dry lapse rates.
            dry = metpy.calc.dry_lapse(pressure=pres, temperature=temp[0])

            # Moist lapse rate.
            moist = metpy.calc.moist_lapse(pressure=pres, temperature=temp[0], reference_pressure=900 * units.hPa)
            ...
            surface_lcl_pressure, surface_lcl_temp = metpy.calc.lcl(pressure=pres[0], temperature=temp[0], dewpoint=dewpoint[0])

            # LFC for surface-based parcel.
            surface_lfc_pressure, surface_lfc_temp = metpy.calc.lfc(
                pressure=surf_pres,
            ...
            surface_el_pressure, surface_el_temp = metpy.calc.el(
                pressure=surf_pres,
                temperature=surf_temp,
                dewpoint=surf_dewpoint,
                parcel_temperature_profile=surface_profile,
            ...
            mixed_cape, mixed_cin = metpy.calc.mixed_layer_cape_cin(pressure=pres, temperature=temp, dewpoint=dewpoint, depth=100 * units.hPa)

            # CAPE and CIN for the most unstable parcel profile in the lowest 300 hPa.
            max_cape, max_cin = metpy.calc.most_unstable_cape_cin(pressure=pres, temperature=temp, dewpoint=dewpoint, depth=300 * units.hPa)
            ...
            lifted_index = metpy.calc.lifted_index(pressure=int_pres, temperature=int_temp, parcel_profile=int_mixed_profile)

            # Deep convection index for lowest 100 hPa mixed parcel.
            dci = int_temp[1].to(units.degC) + int_dewpoint[1].to(units.degC).magnitude * units.delta_degC - lifted_index.to(units.delta_degC)
            ...
            out.append(
                xarray.Dataset(
                    data_vars={
                        'dewpoint': (['model_level_number'], dewpoint.m),
                    ...
                    coords={'model_level_number': dat.pressure.model_level_number, 'latitude': lat, 'longitude': lon},
                ),
            )
...
def conv_properties_xarray(dat, vert_dim='model_level_number', virt_temp=True, lcl_interp='log', pos_cape_neg_cin=False, post_zero_cin=False):
    ...
    Arguments:
       ...
       lcl_interp: Interpolation type to use for LCL.
    ...
    mp = mp.rename({'pressure': 'mp_pressure', 'temperature': 'mp_temperature', 'dewpoint': 'mp_dewpoint'})

    # Surface-based dry adiabats at each point.
    dry = parcel.dry_lapse(pressure=dat.pressure, parcel_temperature=dat.temperature.isel(model_level_number=0))
    dry.name = 'dry_lapse_temp'

    # Surface-based moist adiabats at each point.
    moist = parcel.moist_lapse(pressure=dat.pressure, parcel_temperature=dat.temperature.isel(model_level_number=0), parcel_pressure=900)
    moist.name = 'moist_lapse_temp'

    # Mixed-parcel CAPE and CIN.
    mixed_cape_cin, mixed_profile, _ = parcel.mixed_layer_cape_cin(
        pressure=dat.pressure,
        temperature=dat.temperature,
    ...
    max_cape_cin, _, _ = parcel.most_unstable_cape_cin(
        pressure=dat.pressure,
        temperature=dat.temperature,
        dewpoint=dat.dewpoint,
        depth=300,
    ...
    surface_profile = parcel.parcel_profile_with_lcl(
        pressure=dat.pressure,
        temperature=dat.temperature,
        dewpoint=dat.dewpoint,
    ...
    # LFC for surface-based parcel.
    surface_lfc_el = parcel.lfc_el(
        pressure=surface_profile.pressure,
    ...
    surface_lfc_el = surface_lfc_el.rename(
        {
            'lfc_pressure': 'surface_lfc_pressure',
    ...
    # Surface-based CAPE and CIN.
    surface_cape_cin, surface_profile = parcel.surface_based_cape_cin(
        pressure=dat.pressure,
        temperature=dat.temperature,
        dewpoint=dat.dewpoint,
        virtual_temperature_correction=virt_temp,
    ...
    dci = parcel.deep_convective_index(
        pressure=dat.pressure,
        temperature=dat.temperature,
        dewpoint=dat.dewpoint,
    ...
    wb = parcel.wet_bulb_temperature(pressure=dat.pressure, temperature=dat.temperature, dewpoint=dat.dewpoint)
    wb.name = 'wet_bulb_temperature'

    # Fast web-bulb temperature calculation.
    wb_fast = parcel.wet_bulb_temperature_fast(temperature=dat.temperature, dewpoint=dat.dewpoint)
    wb_fast.name = 'wet_bulb_temperature_fast'

    # Rename clashing variables.
    surface_profile = surface_profile.rename(
        {
            'pressure': 'surf_pres',
...
def test_parcel_functions(dat, virt_temp=False, lcl_interp='log', pos_cape_neg_cin=False, post_zero_cin=False):
    ...
    xarray_results, time = time_function(
        func=conv_properties_xarray,
        dat=dat,
        virt_temp=virt_temp,
        lcl_interp=lcl_interp,
        pos_cape_neg_cin=pos_cape_neg_cin,
...
def benchmark_cape(dat, points=None):
    """Calculate CAPE and CIN using MetPy and xarray functions for a variety of numbers of points and return processing times.
    ...
    for p in points:
        ...
        _, xr_time = time_function(func=surface_cape_vector, dat=pts.copy().persist())
        _, xr_load_time = time_function(func=surface_cape_vector, dat=pts.copy().load())
        _, sr_time = time_function(func=surface_cape_serial, dat=pts.copy().load())
    ...
    return xarray.Dataset({'xr_persist': ('pts', xr_times), 'xr_load': ('pts', xr_load_times), 'serial': ('pts', sr_times)}, {'pts': num_points})
